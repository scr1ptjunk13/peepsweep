...
impl PangolinDex {
    ...
    fn get_chain_config(&self, chain: &str) -> Result<ChainConfig, DexError> {
        match chain.to_lowercase().as_str() {
            "avalanche" => Ok(ChainConfig {
                ...
                router_address: "0xE54Ca86531e17Ef3616d22Ca28b0D458b6C89106".to_string(), // Pangolin Router
                factory_address: "0xefa94DE7a4656D787667C749f7E1223D71E9FD88".to_string(), // Pangolin Factory
                token_list_url: "https://raw.githubusercontent.com/pangolindex/tokenlists/main/pangolin.tokenlist.json".to_string(),
            }),
            _ => Err(DexError::UnsupportedChain(format!("Chain {} not supported by Pangolin", chain))),
        }
    }
    ...
    pub async fn fetch_token_list(&self, chain: &str) -> Result<Vec<TokenInfo>, DexError> {
        ...
        debug!("Fetching Pangolin token list from: {}", config.token_list_url);
    ...
    /// Get Pangolin quote using getAmountsOut
    async fn get_pangolin_quote(&self, params: &QuoteParams) -> Result<(String, u64), DexError> {
        let chain = params.chain.as_deref().unwrap_or("avalanche");
        
        // Validate chain support
        if !self.supported_chains.contains(&chain.to_string()) {
            return Err(DexError::UnsupportedChain(format!("Pangolin doesn't support chain: {}", chain)));
        }
        ...
        // TODO: Implement actual Pangolin Router getAmountsOut call
        // For now, return a placeholder quote
        warn!("Pangolin quote implementation pending - returning placeholder");
        ...
        // Placeholder: assume 0.3% fee
        let output_wei = (input_wei as f64 * 0.997) as u128;
        
        Ok((output_wei.to_string(), self.estimated_gas(chain)))
    }

    /// Get estimated gas for Pangolin swaps
    pub fn estimated_gas(&self, chain: &str) -> u64 {
        match chain.to_lowercase().as_str() {
            "avalanche" => 130_000, // Avalanche gas costs
            _ => 130_000,
        }
    }
...
#[async_trait]
impl DexIntegration for PangolinDex {
    ...
    async fn get_quote(&self, params: &QuoteParams) -> Result<RouteBreakdown, DexError> {
        let chain = params.chain.as_deref().unwrap_or("avalanche");
        info!("🐧 Pangolin get_quote called for {} {} -> {} on {}",
              params.amount_in, params.token_in, params.token_out, chain);

        let (quote, gas_used) = self.get_pangolin_quote(params).await?;
    ...
    async fn is_pair_supported(&self, token_in: &str, token_out: &str, chain: &str) -> Result<bool, DexError> {
        // Check if chain is supported (Pangolin only on Avalanche)
        if chain != "avalanche" {
            return Ok(false);
        }
        ...
        match tokio::time::timeout(
            ...
            Ok(_) => {
                debug!("Pair {}/{} not supported on avalanche via Pangolin", token_in, token_out);
                Ok(false)
            }
            Err(_) => {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_pangolin_initialization() {
        let dex = PangolinDex::new();
        assert_eq!(dex.get_name(), "Pangolin");
    ...
    async fn test_chain_config() {
        ...
        assert_eq!(avalanche_config.router_address, "0xE54Ca86531e17Ef3616d22Ca28b0D458b6C89106");
    ...
    async fn test_gas_estimates() {
        let dex = PangolinDex::new();
        assert_eq!(dex.estimated_gas("avalanche"), 130_000);
    }
}
