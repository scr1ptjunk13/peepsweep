...
impl MaverickDex {
    ...
    fn get_chain_config(&self, chain: &str) -> Result<ChainConfig, DexError> {
        match chain.to_lowercase().as_str() {
            "ethereum" => Ok(ChainConfig {
                ...
                router_address: "0x32C1A6e4142B11EbS2d3B165a75C2F3A73C5d5d0".to_string(), // Maverick Router
                factory_address: "0xEb6625D65a0553c9dBc64449e56abFe519bd9c9B".to_string(), // Maverick Factory
                token_list_url: "https://tokens.coingecko.com/ethereum/all.json".to_string(),
            }),
            "base" => Ok(ChainConfig {
                chain_id: 8453,
                rpc_url: std::env::var("BASE_RPC_URL")
                    .unwrap_or_else(|_| "https://mainnet.base.org".to_string()),
                router_address: "0x32C1A6e4142B11EbS2d3B165a75C2F3A73C5d5d0".to_string(), // Maverick Router on Base
                factory_address: "0xEb6625D65a0553c9dBc64449e56abFe519bd9c9B".to_string(), // Maverick Factory on Base
                token_list_url: "https://tokens.coingecko.com/base/all.json".to_string(),
            }),
            _ => Err(DexError::UnsupportedChain(format!("Chain {} not supported by Maverick", chain))),
        }
    }
    ...
    pub async fn fetch_token_list(&self, chain: &str) -> Result<Vec<TokenInfo>, DexError> {
        ...
        debug!("Fetching Maverick token list from: {}", config.token_list_url);
        ...
        // Add native tokens as special cases
        match chain {
            "ethereum" => {
    ...
    /// Get Maverick quote using concentrated liquidity calculations
    async fn get_maverick_quote(&self, params: &QuoteParams) -> Result<(String, u64), DexError> {
        let chain = params.chain.as_deref().unwrap_or("ethereum");
        
        // Validate chain support
        if !self.supported_chains.contains(&chain.to_string()) {
            return Err(DexError::UnsupportedChain(format!("Maverick doesn't support chain: {}", chain)));
        }
        ...
        // TODO: Implement actual Maverick concentrated liquidity calculations
        // For now, return a placeholder quote
        warn!("Maverick quote implementation pending - returning placeholder");
        ...
        // Placeholder: assume 0.05% fee with concentrated liquidity benefits
        let output_wei = (input_wei as f64 * 0.9995) as u128;
        
        Ok((output_wei.to_string(), self.estimated_gas(chain)))
    }

    /// Get estimated gas for Maverick swaps
    pub fn estimated_gas(&self, chain: &str) -> u64 {
        match chain.to_lowercase().as_str() {
            "ethereum" => 140_000, // Concentrated liquidity is efficient
            "base" => 120_000,     // Base has lower gas costs
            _ => 140_000,
        }
    }
...
#[async_trait]
impl DexIntegration for MaverickDex {
    ...
    async fn get_quote(&self, params: &QuoteParams) -> Result<RouteBreakdown, DexError> {
        let chain = params.chain.as_deref().unwrap_or("ethereum");
        info!("⚡ Maverick get_quote called for {} {} -> {} on {}",
              params.amount_in, params.token_in, params.token_out, chain);

        let (quote, gas_used) = self.get_maverick_quote(params).await?;
    ...
    async fn is_pair_supported(&self, token_in: &str, token_out: &str, chain: &str) -> Result<bool, DexError> {
        // Check if chain is supported (Maverick only on Ethereum)
        if chain != "ethereum" {
            return Ok(false);
        }

        // Check if tokens exist on Ethereum
        match tokio::time::timeout(
            ...
            Ok(_) => {
                debug!("Pair {}/{} not supported on ethereum via Maverick", token_in, token_out);
                Ok(false)
            }
            Err(_) => {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_maverick_initialization() {
        let dex = MaverickDex::new();
        assert_eq!(dex.get_name(), "Maverick");
    ...
    async fn test_gas_estimates() {
        let dex = MaverickDex::new();
        assert_eq!(dex.estimated_gas("ethereum"), 140_000);
        assert_eq!(dex.estimated_gas("base"), 120_000);
    }
}
