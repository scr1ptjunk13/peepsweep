...
impl SpookySwapDex {
    ...
    fn get_chain_config(&self, chain: &str) -> Result<ChainConfig, DexError> {
        match chain.to_lowercase().as_str() {
            "fantom" => Ok(ChainConfig {
                ...
                router_address: "0xF491e7B69E4244ad4002BC14e878a34207E38c29".to_string(), // SpookySwap Router
                factory_address: "0x152eE697f2E276fA89E96742e9bB9aB1F2E61bE3".to_string(), // SpookySwap Factory
            ...
            _ => Err(DexError::UnsupportedChain(format!("Chain {} not supported by SpookySwap", chain))),
        }
    }
    ...
    pub async fn fetch_token_list(&self, chain: &str) -> Result<Vec<TokenInfo>, DexError> {
        ...
        debug!("Fetching SpookySwap token list from: {}", config.token_list_url);
    ...
    /// Get SpookySwap quote using Router's getAmountsOut - following contract-first pattern
    async fn get_spookyswap_quote(&self, params: &QuoteParams) -> Result<String, DexError> {
        // Validate chain support
        let chain = params.chain.as_deref().unwrap_or("fantom");
        if !self.supported_chains.contains(&chain.to_string()) {
            return Err(DexError::UnsupportedChain(format!("SpookySwap doesn't support chain: {}", chain)));
        }
        ...
        let token_in_address = if token_in_addr == "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE" {
            // Convert FTM to WFTM for SpookySwap contracts
        ...
        // Try multiple routes concurrently - SpookySwap is Uniswap V2 fork, so try direct and via WFTM
    ...
    /// Call SpookySwap Router's getAmountsOut function - following Uniswap manual ABI encoding pattern
    async fn try_quote_with_path(
        ...
        // Array offset (0x40 = 64 bytes from start, after amountIn parameter)
        let array_offset = "40";
        
        // Array length
        let array_length = format!("{:x}", path_count);
        
        // Array elements
        let mut array_elements = String::new();
        for addr in &path {
            array_elements.push_str(&format!("{:0>64}", hex::encode(addr.as_slice())));
        }
        ...
        let call_data = format!(
            ...
            format!("{:0>64}", array_offset), // offset to array
            format!("{:0>64}", array_length), // array length
    ...
    /// Get estimated gas for SpookySwap swaps
    pub fn estimated_gas(&self, chain: &str) -> u64 {
        match chain.to_lowercase().as_str() {
            "fantom" => 100_000, // Fantom has very low gas costs
            _ => 100_000,
        }
    }
}
...
#[async_trait]
impl DexIntegration for SpookySwapDex {
    ...
    async fn get_quote(&self, params: &QuoteParams) -> Result<RouteBreakdown, DexError> {
        let amount_out = self.get_spookyswap_quote(params).await?;
    ...
    async fn is_pair_supported(&self, token_in: &str, token_out: &str, chain: &str) -> Result<bool, DexError> {
        // Check if chain is supported (SpookySwap only on Fantom)
        if chain != "fantom" {
            return Ok(false);
        }
        ...
        match tokio::time::timeout(
            ...
            Ok(_) => {
                debug!("Pair {}/{} not supported on fantom via SpookySwap", token_in, token_out);
                Ok(false)
            }
            Err(_) => {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_spookyswap_initialization() {
        let dex = SpookySwapDex::new().await.unwrap();
        assert_eq!(dex.get_name(), "SpookySwap");
    ...
    async fn test_chain_config() {
        ...
        assert_eq!(fantom_config.router_address, "0xF491e7B69E4244ad4002BC14e878a34207E38c29");
