...
impl FraxswapDex {
    ...
    fn get_chain_config(&self, chain: &str) -> Result<ChainConfig, DexError> {
        match chain.to_lowercase().as_str() {
            "ethereum" => Ok(ChainConfig {
                ...
                router_address: "0xC14d550632db8592D1243Edc8B95b0Ad06703867".to_string(), // Fraxswap Router
                factory_address: "0x43eC799eAdd63848443E2347C49f5f52e8Fe0F6f".to_string(), // Fraxswap Factory
                token_list_url: "https://raw.githubusercontent.com/FraxFinance/frax-solidity/master/src/hardhat/misc_constants/token_lists/fraxswap.tokenlist.json".to_string(),
            }),
            _ => Err(DexError::UnsupportedChain(format!("Chain {} not supported by Fraxswap", chain))),
        }
    }
    ...
    pub async fn fetch_token_list(&self, chain: &str) -> Result<Vec<TokenInfo>, DexError> {
        ...
        debug!("Fetching Fraxswap token list from: {}", config.token_list_url);
    ...
    /// Get Fraxswap quote using getAmountsOut
    async fn get_fraxswap_quote(&self, params: &QuoteParams) -> Result<(String, u64), DexError> {
        let chain = params.chain.as_deref().unwrap_or("ethereum");
        
        // Validate chain support
        if !self.supported_chains.contains(&chain.to_string()) {
            return Err(DexError::UnsupportedChain(format!("Fraxswap doesn't support chain: {}", chain)));
        }
        ...
        // TODO: Implement actual Fraxswap Router getAmountsOut call
        // For now, return a placeholder quote
        warn!("Fraxswap quote implementation pending - returning placeholder");
        ...
        // Placeholder: assume 0.3% fee with TWAMM benefits
        let output_wei = (input_wei as f64 * 0.997) as u128;
        
        Ok((output_wei.to_string(), self.estimated_gas(chain)))
    }

    /// Get estimated gas for Fraxswap swaps
    pub fn estimated_gas(&self, chain: &str) -> u64 {
        match chain.to_lowercase().as_str() {
            "ethereum" => 155_000, // TWAMM can be slightly more expensive
            _ => 155_000,
        }
    }
...
#[async_trait]
impl DexIntegration for FraxswapDex {
    ...
    async fn get_quote(&self, params: &QuoteParams) -> Result<RouteBreakdown, DexError> {
        let chain = params.chain.as_deref().unwrap_or("ethereum");
        info!("🏦 Fraxswap get_quote called for {} {} -> {} on {}",
              params.amount_in, params.token_in, params.token_out, chain);

        let (quote, gas_used) = self.get_fraxswap_quote(params).await?;
    ...
    async fn is_pair_supported(&self, token_in: &str, token_out: &str, chain: &str) -> Result<bool, DexError> {
        // Check if chain is supported (Fraxswap only on Ethereum)
        if chain != "ethereum" {
            return Ok(false);
        }
        ...
        match tokio::time::timeout(
            ...
            Ok(_) => {
                debug!("Pair {}/{} not supported on ethereum via Fraxswap", token_in, token_out);
                Ok(false)
            }
            Err(_) => {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_fraxswap_initialization() {
        let dex = FraxswapDex::new();
        assert_eq!(dex.get_name(), "Fraxswap");
    ...
    async fn test_chain_config() {
        ...
        assert_eq!(eth_config.router_address, "0xC14d550632db8592D1243Edc8B95b0Ad06703867");
    ...
    async fn test_gas_estimates() {
        let dex = FraxswapDex::new();
        assert_eq!(dex.estimated_gas("ethereum"), 155_000);
    }
}
