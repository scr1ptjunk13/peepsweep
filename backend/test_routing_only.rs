use bralaladex_backend::routing::{
    UserPreferenceManager, RoutingPreferences, OptimizationStrategy, MevProtectionLevel,
    StrategyTemplateManager, RiskLevel
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    preferences.max_slippage_tolerance = Decimal::from_str("0.005")?; // 0.5%
    preferences.min_liquidity_threshold = Decimal::from_str("100000")?; // $100k
    preferences.gas_vs_price_preference = Decimal::from_str("0.7")?; // 70% price focus
    ...
    assert_eq!(retrieved_prefs.max_slippage_tolerance, Decimal::from_str("0.005")?);
    ...
    let trade_amount = Decimal::from_str("10000")?; // $10k trade
    ...
    assert_eq!(default_prefs.max_slippage_tolerance, Decimal::from_str("0.5")?); // 0.5%
